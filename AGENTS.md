...
- Regenerate `gists/**` with `pnpm gists:package` after source edits.
- Keep the generated gist package check passing with `pnpm gists:check`.
- Keep GitHub Actions generated by `@async/pipeline`; do not hand-edit `.github/workflows/async-pipeline.yml`.
- If `pipeline.ts` changes, run `pnpm pipeline:github:generate` and `pnpm pipeline:github:check`.
